...
class FeaturesExtractor(object):
	...
	def exract_markup_data_track1(self, markup_doc_list, participants_doc_list, output_file_name, data_type, 
								output_collection):
		...
		for doc in corpus.load_documents():
			...
			for participant in participant_outputs:
				...
				if participant.has_output(doc.document_id):
					found_objects = participant.get_track_output(doc.document_id, 
	...
	def exract_data_track1(self, corpus, doc_names, participants_dir, output_file_name, 
		...
		for doc in doc_names:
			# Копируем информацию о токенах документа
			try:
				doc_tokens = list(corpus.get_document(doc).tokens.values())
			except:
				continue
			tokens_start_pos = list(map(lambda x: x.pos, doc_tokens))
			doc_tokens.sort(key = lambda x: x.pos)
